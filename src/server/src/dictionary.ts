export let replacementsMap = new Map([
	["learning disabled","person with learning disabilities"],
	["disabled","turned off, has a disability, person with a disability, people with disabilities"],
	["invalid","turned off, has a disability, person with a disability, people with disabilities"],
	["birth defect","has a disability, person with a disability, people with disabilities"],
	["suffers from disabilities","has a disability, person with a disability, people with disabilities"],
	["suffering from disabilities","has a disability, person with a disability, people with disabilities"],
	["suffering from a disability","has a disability, person with a disability, people with disabilities"],
	["afflicted with disabilities","has a disability, person with a disability, people with disabilities"],
	["afflicted with a disability","has a disability, person with a disability, people with disabilities"],
	["intellectually disabled people","people with intellectual disabilities"],
	["intellectually disabled","person with an intellectual disability"],
	["has intellectual issues","person with an intellectual disability"],
	["suffers from intellectual disabilities","person with an intellectual disability"],
	["suffering from intellectual disabilities","person with an intellectual disability"],
	["suffering from an intellectual disability","person with an intellectual disability"],
	["afflicted with intellectual disabilities","person with an intellectual disability"],
	["afflicted with a intellectual disability","person with an intellectual disability"],
	["batshit","rude, malicious, mean, disgusting, vile, person with symptoms of mental illness, person with mental illness, person with symptoms of a mental disorder, person with a mental disorder"],
	["psycho","rude, malicious, mean, disgusting, vile, person with symptoms of mental illness, person with mental illness, person with symptoms of a mental disorder, person with a mental disorder"],
	["crazy","rude, malicious, mean, disgusting, vile, person with symptoms of mental illness, person with mental illness, person with symptoms of a mental disorder, person with a mental disorder"],
	["delirious","rude, malicious, mean, disgusting, vile, person with symptoms of mental illness, person with mental illness, person with symptoms of a mental disorder, person with a mental disorder"],
	["insane","rude, malicious, mean, disgusting, vile, person with symptoms of mental illness, person with mental illness, person with symptoms of a mental disorder, person with a mental disorder"],
	["insanity","rude, malicious, mean, disgusting, vile, person with symptoms of mental illness, person with mental illness, person with symptoms of a mental disorder, person with a mental disorder"],
	["loony","rude, malicious, mean, disgusting, vile, person with symptoms of mental illness, person with mental illness, person with symptoms of a mental disorder, person with a mental disorder"],
	["lunacy","rude, malicious, mean, disgusting, vile, person with symptoms of mental illness, person with mental illness, person with symptoms of a mental disorder, person with a mental disorder"],
	["lunatic","rude, malicious, mean, disgusting, vile, person with symptoms of mental illness, person with mental illness, person with symptoms of a mental disorder, person with a mental disorder"],
	["mentally ill","rude, malicious, mean, disgusting, vile, person with symptoms of mental illness, person with mental illness, person with symptoms of a mental disorder, person with a mental disorder"],
	["psychopathology","rude, malicious, mean, disgusting, vile, person with symptoms of mental illness, person with mental illness, person with symptoms of a mental disorder, person with a mental disorder"],
	["mental defective","rude, malicious, mean, disgusting, vile, person with symptoms of mental illness, person with mental illness, person with symptoms of a mental disorder, person with a mental disorder"],
	["moron","rude, malicious, mean, disgusting, vile, person with symptoms of mental illness, person with mental illness, person with symptoms of a mental disorder, person with a mental disorder"],
	["moronic","rude, malicious, mean, disgusting, vile, person with symptoms of mental illness, person with mental illness, person with symptoms of a mental disorder, person with a mental disorder"],
	["nuts","rude, malicious, mean, disgusting, vile, person with symptoms of mental illness, person with mental illness, person with symptoms of a mental disorder, person with a mental disorder"],
	["mental case","rude, malicious, mean, disgusting, vile, person with symptoms of mental illness, person with mental illness, person with symptoms of a mental disorder, person with a mental disorder"],
	["mental","rude, malicious, mean, disgusting, vile, person with symptoms of mental illness, person with mental illness, person with symptoms of a mental disorder, person with a mental disorder"],
	["sane","correct, adequate, sufficient, consistent, valid, coherent, sensible, reasonable"],
	["sanity check","check, assertion, validation, smoke test"],
	["bipolar","fluctuating, person with bipolar disorder"],
	["schizophrenic","person with schizophrenia"],
	["schizo","person with schizophrenia"],
	["suffers from schizophrenia","person with schizophrenia"],
	["suffering from schizophrenia","person with schizophrenia"],
	["afflicted with schizophrenia","person with schizophrenia"],
	["manic","person with schizophrenia"],
	["handicapped parking","accessible parking"],
	["handicapped","person with a handicap, accessible"],
	["amputee","person with an amputation"],
	["cripple","person with a limp"],
	["crippled","person with a limp"],
	["gimp","person with a limp"],
	["mongoloid","person with Down Syndrome"],
	["stroke victim","individual who has had a stroke"],
	["suffering from a stroke","individual who has had a stroke"],
	["victim of a stroke","individual who has had a stroke"],
	["suffers from multiple sclerosis","person who has multiple sclerosis"],
	["suffering from multiple sclerosis","person who has multiple sclerosis"],
	["victim of multiple sclerosis","person who has multiple sclerosis"],
	["multiple sclerosis victim","person who has multiple sclerosis"],
	["afflicted with multiple sclerosis","person who has multiple sclerosis"],
	["suffers from muscular dystrophy","person who has muscular dystrophy"],
	["afflicted with muscular dystrophy","person who has muscular dystrophy"],
	["suffers from MD","person who has muscular dystrophy"],
	["afflicted with MD","person who has muscular dystrophy"],
	["family burden","with family support needs"],
	["asylum","psychiatric hospital, mental health hospital"],
	["bedlam","chaos, hectic, pandemonium"],
	["madhouse","chaos, hectic, pandemonium"],
	["loony bin","chaos, hectic, pandemonium"],
	["downs syndrome","Down Syndrome"],
	["retard","silly, dullard, person with Down Syndrome, person with developmental disabilities, delay, hold back"],
	["retarded","silly, dullard, person with Down Syndrome, person with developmental disabilities, delay, hold back"],
	["short bus","silly, dullard, person with Down Syndrome, person with developmental disabilities, delay, hold back"],
	["retards","sillies, dullards, people with developmental disabilities, people with Down’s Syndrome, delays, holds back"],
	["psychotic","person with a psychotic condition, person with psychosis"],
	["suffers from psychosis","person with a psychotic condition, person with psychosis"],
	["suffering from psychosis","person with a psychotic condition, person with psychosis"],
	["afflicted with psychosis","person with a psychotic condition, person with psychosis"],
	["victim of psychosis","person with a psychotic condition, person with psychosis"],
	["lame","boring, dull"],
	["suffering from aids","person with AIDS"],
	["suffer from aids","person with AIDS"],
	["suffers from aids","person with AIDS"],
	["afflicted with aids","person with AIDS"],
	["victim of aids","person with AIDS"],
	["aids victim","person with AIDS"],
	["confined to a wheelchair","uses a wheelchair"],
	["bound to a wheelchair","uses a wheelchair"],
	["restricted to a wheelchair","uses a wheelchair"],
	["wheelchair bound","uses a wheelchair"],
	["special olympians","athletes, Special Olympics athletes"],
	["special olympic athletes","athletes, Special Olympics athletes"],
	["ablebodied","non-disabled"],
	["addict","person with a drug addiction, person recovering from a drug addiction"],
	["junkie","person with a drug addiction, person recovering from a drug addiction"],
	["addicts","people with a drug addiction, people recovering from a drug addiction"],
	["junkies","people with a drug addiction, people recovering from a drug addiction"],
	["alcoholic","someone with an alcohol problem"],
	["alcohol abuser","someone with an alcohol problem"],
	["deaf and dumb","deaf"],
	["deafmute","deaf"],
	["demented","person with dementia"],
	["senile","person with dementia"],
	["depressed","sad, blue, bummed out, person with seasonal affective disorder, person with psychotic depression, person with postpartum depression"],
	["vertically challenged","person with dwarfism, little person, little people, LP, person of short stature"],
	["midget","person with dwarfism, little person, little people, LP, person of short stature"],
	["small person","person with dwarfism, little person, little people, LP, person of short stature"],
	["dwarf","person with dwarfism, little person, little people, LP, person of short stature"],
	["dyslexic","person with dyslexia"],
	["epileptic","person with epilepsy"],
	["hearing impaired","hard of hearing, partially deaf, partial hearing loss, deaf"],
	["hearing impairment","hard of hearing, partially deaf, partial hearing loss, deaf"],
	["infantile paralysis","polio, person who had polio"],
	["suffers from polio","polio, person who had polio"],
	["suffering from polio","polio, person who had polio"],
	["suffering from a polio","polio, person who had polio"],
	["afflicted with polio","polio, person who had polio"],
	["afflicted with a polio","polio, person who had polio"],
	["victim of polio","polio, person who had polio"],
	["suffer from an injury","sustain an injury, receive an injury"],
	["suffers from an injury","sustain an injury, receive an injury"],
	["suffering from an injury","sustain an injury, receive an injury"],
	["afflicted with an injury","sustain an injury, receive an injury"],
	["victim of an injury","sustain an injury, receive an injury"],
	["suffer from injuries","sustain injuries, receive injuries"],
	["suffers from injuries","sustain injuries, receive injuries"],
	["suffering from injuries","sustain injuries, receive injuries"],
	["afflicted with injuries","sustain injuries, receive injuries"],
	["victim of injuries","sustain injuries, receive injuries"],
	["paraplegic","person with paraplegia"],
	["quadriplegic","person with quadriplegia"],
	["spaz","person with cerebral palsy, twitch, flinch, hectic"],
	["spastic","person with cerebral palsy, twitch, flinch"],
	["stammering","stuttering, disfluency of speech"],
	["stutterer","person who stutters"],
	["tourettes syndrome","Tourette syndrome"],
	["tourettes disorder","Tourette syndrome"],
	["rehab center","treatment center"],
	["detox center","treatment center"],
	["rehab","treatment"],
	["detox","treatment"],
	["sociopath","person with a personality disorder, person with psychopathic personality"],
	["sociopaths","people with psychopathic personalities, people with a personality disorder"],
	["dumb","foolish, ludicrous, speechless, silent"],
	["simpleton","foolish, ludicrous, unintelligent"],
	["stupid","foolish, ludicrous, unintelligent"],
	["wacko","foolish, ludicrous, unintelligent"],
	["whacko","foolish, ludicrous, unintelligent"],
	["low iq","foolish, ludicrous, unintelligent"],
	["panic attack","fit of terror, scare"],
	["anorexic","thin, slim"],
	["bony","thin, slim"],
	["neurotic","has an anxiety disorder, obsessive, pedantic, niggly, picky"],
	["ocd","has an anxiety disorder, obsessive, pedantic, niggly, picky"],
	["o.c.d","has an anxiety disorder, obsessive, pedantic, niggly, picky"],
	["o.c.d.","has an anxiety disorder, obsessive, pedantic, niggly, picky"],
	["insomnia","restlessness, sleeplessness"],
	["insomniac","person who has insomnia"],
	["insomniacs","people who have insomnia"],
	["barren","empty, sterile, infertile"],
	["blind to","careless, heartless, indifferent, insensitive"],
	["blind eye to","careless, heartless, indifferent, insensitive"],
	["blinded by","careless, heartless, indifferent, insensitive"],
	["deaf to","careless, heartless, indifferent, insensitive"],
	["deaf ear to","careless, heartless, indifferent, insensitive"],
	["deafened by","careless, heartless, indifferent, insensitive"],
	["cretin","creep, fool"],
	["daft","absurd, foolish"],
	["feebleminded","foolish, ludicrous, silly"],
	["feeble minded","foolish, ludicrous, silly"],
	["idiot","foolish, ludicrous, silly"],
	["imbecile","foolish, ludicrous, silly"],
	["harelipped","person with a cleft-lip and palate"],
	["cleftlipped","person with a cleft-lip and palate"],
	["harelip","cleft-lip and palate"],
	["hare lip","cleft-lip and palate"],
	["maniac","fanatic, zealot, enthusiast"],
	["bucktoothed","person with prominent teeth, prominent teeth"],
	["buckteeth","person with prominent teeth, prominent teeth"],
	["challenged","has a disability, person with a disability, people with disabilities"],
	["diffability","has a disability, person with a disability, people with disabilities"],
	["differently abled","has a disability, person with a disability, people with disabilities"],
	["handicapable","has a disability, person with a disability, people with disabilities"],
	["special","has a disability, person with a disability, people with disabilities"],
	["special needs","has a disability, person with a disability, people with disabilities"],
	["specially abled","has a disability, person with a disability, people with disabilities"],
	["fucktard","disagreeable, uneducated, ignorant, naive, inconsiderate"],
	["libtard","disagreeable, uneducated, ignorant, naive, inconsiderate"],
	["contard","disagreeable, uneducated, ignorant, naive, inconsiderate"],
	["ADD","disorganized, distracted, energetic, hyperactive, impetuous, impulsive, inattentive, restless, unfocused"],
	["adhd","disorganized, distracted, energetic, hyperactive, impetuous, impulsive, inattentive, restless, unfocused"],
	["a.d.d.","disorganized, distracted, energetic, hyperactive, impetuous, impulsive, inattentive, restless, unfocused"],
	["a.d.h.d.","disorganized, distracted, energetic, hyperactive, impetuous, impulsive, inattentive, restless, unfocused"],
	["her","their, theirs, them"],
	["hers","their, theirs, them"],
	["him","their, theirs, them"],
	["his","their, theirs, them"],
	["she","they, it"],
	["he","they, it"],
	["she'd","they, it"],
	["he'd","they, it"],
	["she'll","they, it"],
	["he'll","they, it"],
	["she's","they, it"],
	["he's","they, it"],
	["herself","themselves, theirself, self"],
	["himself","themselves, theirself, self"],
	["girl","kid, child, youth"],
	["boy","kid, child, youth"],
	["women","people, persons, folks"],
	["girls","people, persons, folks"],
	["gals","people, persons, folks"],
	["ladies","people, persons, folks"],
	["men","people, persons, folks"],
	["guys","people, persons, folks"],
	["dudes","people, persons, folks"],
	["gents","people, persons, folks"],
	["gentlemen","people, persons, folks"],
	["woman","person, friend, pal, folk, individual"],
	["gal","person, friend, pal, folk, individual"],
	["lady","person, friend, pal, folk, individual"],
	["babe","person, friend, pal, folk, individual"],
	["bimbo","person, friend, pal, folk, individual"],
	["chick","person, friend, pal, folk, individual"],
	["guy","person, friend, pal, folk, individual"],
	["lad","person, friend, pal, folk, individual"],
	["fellow","person, friend, pal, folk, individual"],
	["dude","person, friend, pal, folk, individual"],
	["bro","person, friend, pal, folk, individual"],
	["gentleman","person, friend, pal, folk, individual"],
	["motherland","native land, homeland"],
	["fatherland","native land, homeland"],
	["mother tongue","native tongue, native language"],
	["father tongue","native tongue, native language"],
	["freshwomen","first-year students, freshers"],
	["freshmen","first-year students, freshers"],
	["garbagewoman","garbage collector, waste collector, trash collector"],
	["garbageman","garbage collector, waste collector, trash collector"],
	["garbagewomen","garbage collectors, waste collectors, trash collectors"],
	["garbagemen","garbage collectors, waste collectors, trash collectors"],
	["chairwoman","chair, head, chairperson, coordinator, committee head, moderator, presiding officer"],
	["chairman","chair, head, chairperson, coordinator, committee head, moderator, presiding officer"],
	["committee woman","committee member"],
	["committee man","committee member"],
	["cowgirl","cowhand"],
	["cowboy","cowhand"],
	["cowgirls","cowhands"],
	["cowboys","cowhands"],
	["cattlewoman","cattle rancher"],
	["cattleman","cattle rancher"],
	["cattlewomen","cattle ranchers"],
	["cattlemen","cattle ranchers"],
	["chairwomen","chairs, chairpersons, coordinators"],
	["chairmen","chairs, chairpersons, coordinators"],
	["postwoman","mail carrier, letter carrier, postal worker"],
	["mailwoman","mail carrier, letter carrier, postal worker"],
	["postman","mail carrier, letter carrier, postal worker"],
	["mailman","mail carrier, letter carrier, postal worker"],
	["postwomen","mail carriers, letter carriers, postal workers"],
	["mailwomen","mail carriers, letter carriers, postal workers"],
	["postmen","mail carriers, letter carriers, postal workers"],
	["mailmen","mail carriers, letter carriers, postal workers"],
	["policewoman","officer, police officer"],
	["policeman","officer, police officer"],
	["chick cop","officer, police officer"],
	["policewomen","officers, police officers"],
	["policemen","officers, police officers"],
	["stewardess","flight attendant"],
	["steward","flight attendant"],
	["stewardesses","flight attendants"],
	["stewards","flight attendants"],
	["congresswoman","member of congress, congress person, legislator, representative"],
	["congressman","member of congress, congress person, legislator, representative"],
	["congresswomen","members of congress, congress persons, legislators, representatives"],
	["congressmen","members of congress, congress persons, legislators, representatives"],
	["firewoman","fire fighter, fire officer"],
	["fireman","fire fighter, fire officer"],
	["firewomen","fire fighters"],
	["firemen","fire fighters"],
	["fisherwoman","fisher, crew member, fisherfolk, angler"],
	["fisherman","fisher, crew member, fisherfolk, angler"],
	["fisherwomen","fishers"],
	["fishermen","fishers"],
	["sisterhood","kinship, community"],
	["brotherhood","kinship, community"],
	["common girl","common person, average person"],
	["common man","common person, average person"],
	["businesswoman","business executive, entrepreneur, business person, professional"],
	["salarywoman","business executive, entrepreneur, business person, professional"],
	["businessman","business executive, entrepreneur, business person, professional"],
	["salaryman","business executive, entrepreneur, business person, professional"],
	["businesswomen","business executives, entrepreneurs"],
	["salarywomen","business executives, entrepreneurs"],
	["career girl","business executives, entrepreneurs"],
	["career woman","business executives, entrepreneurs"],
	["businessmen","business executives, entrepreneurs"],
	["salarymen","business executives, entrepreneurs"],
	["cleaning lady","cleaner"],
	["cleaning girl","cleaner"],
	["cleaning woman","cleaner"],
	["janitress","cleaner"],
	["cleaning man","cleaner"],
	["cleaning boy","cleaner"],
	["janitor","cleaner"],
	["cleaning ladies","cleaners, housekeeping"],
	["cleaning girls","cleaners, housekeeping"],
	["janitresses","cleaners, housekeeping"],
	["cleaning men","cleaners, housekeeping"],
	["janitors","cleaners, housekeeping"],
	["delivery girl","courier, messenger"],
	["delivery boy","courier, messenger"],
	["forewoman","supervisor, shift boss"],
	["foreman","supervisor, shift boss"],
	["frontwoman","lead, front, figurehead"],
	["front woman","lead, front, figurehead"],
	["frontman","lead, front, figurehead"],
	["front man","lead, front, figurehead"],
	["front women","figureheads"],
	["frontwomen","figureheads"],
	["front men","figureheads"],
	["frontmen","figureheads"],
	["forewomen","supervisors, shift bosses"],
	["foremen","supervisors, shift bosses"],
	["insurance woman","insurance agent"],
	["insurance man","insurance agent"],
	["insurance women","insurance agents"],
	["insurance men","insurance agents"],
	["landlady","proprietor, building manager"],
	["landlord","proprietor, building manager"],
	["landladies","proprietors, building managers"],
	["landlords","proprietors, building managers"],
	["alumna","graduate"],
	["alumnus","graduate"],
	["alumnae","graduates"],
	["alumni","graduates"],
	["newswoman","anchor, journalist"],
	["newspaperwoman","anchor, journalist"],
	["anchorwoman","anchor, journalist"],
	["newsman","anchor, journalist"],
	["newspaperman","anchor, journalist"],
	["anchorman","anchor, journalist"],
	["newswomen","anchors, journalists"],
	["newspaperwomen","anchors, journalists"],
	["anchorwomen","anchors, journalists"],
	["newsmen","anchors, journalists"],
	["newspapermen","anchors, journalists"],
	["anchormen","anchors, journalists"],
	["repairwoman","repairer, technician"],
	["repairman","repairer, technician"],
	["repairwomen","technicians"],
	["repairmen","technicians"],
	["saleswoman","salesperson, sales clerk, sales rep, sales agent, sales attendant, seller, shop assistant"],
	["sales woman","salesperson, sales clerk, sales rep, sales agent, sales attendant, seller, shop assistant"],
	["saleslady","salesperson, sales clerk, sales rep, sales agent, sales attendant, seller, shop assistant"],
	["salesman","salesperson, sales clerk, sales rep, sales agent, sales attendant, seller, shop assistant"],
	["sales man","salesperson, sales clerk, sales rep, sales agent, sales attendant, seller, shop assistant"],
	["saleswomen","sales clerks, sales reps, sales agents, sellers"],
	["sales women","sales clerks, sales reps, sales agents, sellers"],
	["salesladies","sales clerks, sales reps, sales agents, sellers"],
	["salesmen","sales clerks, sales reps, sales agents, sellers"],
	["sales men","sales clerks, sales reps, sales agents, sellers"],
	["servicewoman","soldier, service representative"],
	["serviceman","soldier, service representative"],
	["servicewomen","soldiers, service representatives"],
	["servicemen","soldiers, service representatives"],
	["waitress","server"],
	["waiter","server"],
	["waitresses","servers"],
	["waiters","servers"],
	["workwoman","worker, wage earner, taxpayer"],
	["working woman","worker, wage earner, taxpayer"],
	["workman","worker, wage earner, taxpayer"],
	["working man","worker, wage earner, taxpayer"],
	["workwomen","workers"],
	["workmen","workers"],
	["actress","performer, star, artist, entertainer"],
	["actor","performer, star, artist, entertainer"],
	["actresses","performers, stars, artists, entertainers"],
	["actors","performers, stars, artists, entertainers"],
	["aircrewwoman","pilot, aviator, airstaff"],
	["aircrew woman","pilot, aviator, airstaff"],
	["aircrewman","pilot, aviator, airstaff"],
	["airman","pilot, aviator, airstaff"],
	["aircrewwomen","pilots, aviators, airstaff"],
	["aircrew women","pilots, aviators, airstaff"],
	["aircrewmen","pilots, aviators, airstaff"],
	["airmen","pilots, aviators, airstaff"],
	["alderwoman","cabinet member"],
	["alderman","cabinet member"],
	["alderwomen","cabinet, cabinet members, alderperson"],
	["aldermen","cabinet, cabinet members, alderperson"],
	["assemblywoman","assembly person, assembly worker"],
	["assemblyman","assembly person, assembly worker"],
	["kinswoman","relative"],
	["aunt","relative"],
	["kinsman","relative"],
	["uncle","relative"],
	["kinswomen","relatives"],
	["aunts","relatives"],
	["kinsmen","relatives"],
	["uncles","relatives"],
	["boogeywoman","boogeymonster"],
	["boogeyman","boogeymonster"],
	["boogiewoman","boogeymonster"],
	["boogieman","boogeymonster"],
	["bogeywoman","bogeymonster"],
	["bogeyman","bogeymonster"],
	["bogiewoman","bogeymonster"],
	["bogieman","bogeymonster"],
	["boogiewomen","boogeymonsters"],
	["boogiemen","boogeymonsters"],
	["bogiewomen","bogeymonsters"],
	["bogiemen","bogeymonsters"],
	["bondswoman","bonder"],
	["bondsman","bonder"],
	["bondswomen","bonders"],
	["bondsmen","bonders"],
	["wife","partner, significant other, spouse"],
	["husband","partner, significant other, spouse"],
	["wives","partners, significant others, spouses"],
	["husbands","partners, significant others, spouses"],
	["girlfriend","partner, friend, significant other"],
	["boyfriend","partner, friend, significant other"],
	["girlfriends","partners, friends, significant others"],
	["boyfriends","partners, friends, significant others"],
	["girlhood","childhood"],
	["boyhood","childhood"],
	["girly","childish"],
	["girlish","childish"],
	["boyish","childish"],
	["journeywoman","journeyperson"],
	["journeyman","journeyperson"],
	["journeywomen","journeypersons"],
	["journeymen","journeypersons"],
	["godmother","godparent, elder, patron"],
	["patroness","godparent, elder, patron"],
	["godfather","godparent, elder, patron"],
	["granddaughter","grandchild"],
	["grandson","grandchild"],
	["granddaughters","grandchildren"],
	["grandsons","grandchildren"],
	["foremother","ancestor"],
	["forefather","ancestor"],
	["foremothers","ancestors"],
	["forefathers","ancestors"],
	["granny","grandparent, ancestor"],
	["grandma","grandparent, ancestor"],
	["grandmother","grandparent, ancestor"],
	["grandpappy","grandparent, ancestor"],
	["granddaddy","grandparent, ancestor"],
	["gramps","grandparent, ancestor"],
	["grandpa","grandparent, ancestor"],
	["grandfather","grandparent, ancestor"],
	["grandmothers","grandparents, ancestors"],
	["grandfathers","grandparents, ancestors"],
	["bride","spouse, newlywed"],
	["groom","spouse, newlywed"],
	["sister","sibling"],
	["brother","sibling"],
	["sisters","siblings"],
	["brothers","siblings"],
	["camerawoman","camera operator, camera person"],
	["cameraman","camera operator, camera person"],
	["camerawomen","camera operators"],
	["cameramen","camera operators"],
	["cavewoman","troglodyte, hominidae"],
	["caveman","troglodyte, hominidae"],
	["cavewomen","troglodytae, troglodyti, troglodytes, hominids"],
	["cavemen","troglodytae, troglodyti, troglodytes, hominids"],
	["clergywoman","clergyperson, clergy, cleric"],
	["clergyman","clergyperson, clergy, cleric"],
	["clergywomen","clergies, clerics"],
	["clergymen","clergies, clerics"],
	["councilwoman","council member"],
	["councilman","council member"],
	["councilwomen","council members"],
	["councilmen","council members"],
	["countrywoman","country person"],
	["countryman","country person"],
	["countrywomen","country folk"],
	["countrymen","country folk"],
	["handywoman","artisan, craftsperson, skilled worker"],
	["craftswoman","artisan, craftsperson, skilled worker"],
	["handyman","artisan, craftsperson, skilled worker"],
	["craftsman","artisan, craftsperson, skilled worker"],
	["hostess","presenter, entertainer, emcee"],
	["host","presenter, entertainer, emcee"],
	["hostesses","presenters, entertainers, emcees"],
	["hosts","presenters, entertainers, emcees"],
	["handywomen","artisans, craftspersons, skilled workers"],
	["craftswomen","artisans, craftspersons, skilled workers"],
	["handymen","artisans, craftspersons, skilled workers"],
	["craftsmen","artisans, craftspersons, skilled workers"],
	["hangwoman","guillotine"],
	["hangman","guillotine"],
	["hangwomen","guillotines"],
	["hangmen","guillotines"],
	["henchwoman","sidekick"],
	["henchman","sidekick"],
	["henchwomen","sidekicks"],
	["henchmen","sidekicks"],
	["heroine","role-model, mentor"],
	["hero","role-model, mentor"],
	["heroines","role-models, mentor"],
	["heroes","role-models, mentor"],
	["maternal","parental, warm, intimate"],
	["paternal","parental, warm, intimate"],
	["fraternal","parental, warm, intimate"],
	["maternity","parental"],
	["paternity","parental"],
	["mamas","parents"],
	["mothers","parents"],
	["moms","parents"],
	["mums","parents"],
	["mommas","parents"],
	["mommies","parents"],
	["papas","parents"],
	["fathers","parents"],
	["dads","parents"],
	["daddies","parents"],
	["mama","parent"],
	["mother","parent"],
	["mom","parent"],
	["mum","parent"],
	["momma","parent"],
	["mommy","parent"],
	["papa","parent"],
	["father","parent"],
	["dad","parent"],
	["pop","parent"],
	["daddy","parent"],
	["daughter","child"],
	["son","child"],
	["daughters","children"],
	["sons","children"],
	["doorwoman","concierge"],
	["doorman","concierge"],
	["doorwomen","concierges"],
	["doormen","concierges"],
	["feminin","humanly, mature"],
	["dudely","humanly, mature"],
	["manly","humanly, mature"],
	["females","humans"],
	["males","humans"],
	["empress","ruler"],
	["queen","ruler"],
	["emperor","ruler"],
	["king","ruler"],
	["empresses","rulers"],
	["queens","rulers"],
	["emperors","rulers"],
	["kings","rulers"],
	["queensize","jumbo, gigantic"],
	["kingsize","jumbo, gigantic"],
	["queenmaker","power behind the throne"],
	["kingmaker","power behind the throne"],
	["laywoman","civilian"],
	["layman","civilian"],
	["laywomen","civilians"],
	["laymen","civilians"],
	["dame","official, owner, expert, superior, chief, ruler"],
	["lord","official, owner, expert, superior, chief, ruler"],
	["dames","officials, masters, chiefs, rulers"],
	["lords","officials, masters, chiefs, rulers"],
	["womanhood","adulthood, personhood, maturity"],
	["masculinity","adulthood, personhood, maturity"],
	["manhood","adulthood, personhood, maturity"],
	["femininity","humanity"],
	["manliness","humanity"],
	["markswoman","shooter"],
	["marksman","shooter"],
	["markswomen","shooters"],
	["marksmen","shooters"],
	["middlewoman","intermediary, go-between"],
	["middleman","intermediary, go-between"],
	["middlewomen","intermediaries, go-betweens"],
	["middlemen","intermediaries, go-betweens"],
	["milkwoman","milk person"],
	["milkman","milk person"],
	["milkwomen","milk people"],
	["milkmen","milk people"],
	["niece","nibling, sibling’s child"],
	["nephew","nibling, sibling’s child"],
	["nieces","niblings, sibling’s children"],
	["nephews","niblings, sibling’s children"],
	["noblewoman","noble"],
	["nobleman","noble"],
	["noblewomen","nobles"],
	["noblemen","nobles"],
	["ombudswoman","notary, consumer advocate, trouble shooter, omsbudperson, mediator"],
	["ombudsman","notary, consumer advocate, trouble shooter, omsbudperson, mediator"],
	["ombudswomen","notaries, omsbudpersons, omsbudpeople, mediators"],
	["ombudsmen","notaries, omsbudpersons, omsbudpeople, mediators"],
	["princess","heir"],
	["prince","heir"],
	["princesses","heirs"],
	["princes","heirs"],
	["sandwoman","fairy"],
	["sandman","fairy"],
	["sandwomen","fairies"],
	["sandmen","fairies"],
	["showwoman","promoter"],
	["showman","promoter"],
	["showwomen","promoters"],
	["show women","promoters"],
	["showmen","promoters"],
	["spacewoman","astronaut"],
	["spaceman","astronaut"],
	["spacewomen","astronauts"],
	["spacemen","astronauts"],
	["spokeswoman","speaker, spokesperson, representative"],
	["spokesman","speaker, spokesperson, representative"],
	["spokeswomen","speakers, spokespersons"],
	["spokesmen","speakers, spokespersons"],
	["sportswoman","athlete, sports person"],
	["sportsman","athlete, sports person"],
	["sportswomen","athletes, sports persons"],
	["sportsmen","athletes, sports persons"],
	["stateswoman","senator"],
	["statesman","senator"],
	["stepsister","step-sibling"],
	["stepbrother","step-sibling"],
	["stepsisters","step-siblings"],
	["stepbrothers","step-siblings"],
	["stepmom","step-parent"],
	["stepmother","step-parent"],
	["stepdad","step-parent"],
	["stepfather","step-parent"],
	["stepmothers","step-parents"],
	["stepfathers","step-parents"],
	["superwoman","titan"],
	["superman","titan"],
	["superwomen","titans"],
	["supermen","titans"],
	["unwomanly","inhumane"],
	["unwomenly","inhumane"],
	["unmanly","inhumane"],
	["unmenly","inhumane"],
	["watchwoman","watcher"],
	["watchman","watcher"],
	["watchwomen","watchers"],
	["watchmen","watchers"],
	["weatherwoman","weather forecaster, meteorologist"],
	["weatherman","weather forecaster, meteorologist"],
	["weatherwomen","weather forecasters, meteorologists"],
	["weathermen","weather forecasters, meteorologists"],
	["widow","bereaved"],
	["widows","bereaved"],
	["widower","bereaved"],
	["widowers","bereaved"],
	["own woman","own person"],
	["own man","own person"],
	["frenchmen","french, the french"],
	["ladylike","courteous, cultured"],
	["like a man","resolutely, bravely"],
	["maiden name","birth name"],
	["maiden voyage","first voyage"],
	["man enough","strong enough"],
	["oneupmanship","upstaging, competitiveness"],
	["miss.","ms."],
	["mrs.","ms."],
	["manmade","manufactured, artificial, synthetic, machine-made, constructed"],
	["man of action","dynamo"],
	["man of letters","scholar, writer, literary figure"],
	["man of the world","sophisticate"],
	["fellowship","camaraderie, community, organization"],
	["freshman","first-year student, fresher"],
	["freshwoman","first-year student, fresher"],
	["workmanship","quality construction, expertise"],
	["housewife","homemaker, homeworker"],
	["housewives","homemakers, homeworkers"],
	["motherly","loving, warm, nurturing"],
	["manpower","human resources, workforce, personnel, staff, labor, labor force, staffing, combat personnel"],
	["master of ceremonies","emcee, moderator, convenor"],
	["masterful","skilled, authoritative, commanding"],
	["mastermind","genius, creator, instigator, oversee, launch, originate"],
	["masterpiece","work of genius, chef d’oeuvre"],
	["masterplan","vision, comprehensive plan"],
	["masterstroke","trump card, stroke of genius"],
	["madman","fanatic, zealot, enthusiast"],
	["mad man","fanatic, zealot, enthusiast"],
	["madmen","fanatics, zealots, enthusiasts"],
	["mad men","fanatics, zealots, enthusiasts"],
	["mankind","humankind"],
	["manhour","staff hour, hour of work"],
	["man hour","staff hour, hour of work"],
	["manhours","staff hours, hours of work, hours of labor, hours"],
	["man hours","staff hours, hours of work, hours of labor, hours"],
	["manned","staffed, crewed, piloted"],
	["unmanned","robotic, automated"],
	["bitching","whining, complaining, crying"],
	["moaning","whining, complaining, crying"],
	["bitch","whine, complain, cry"],
	["moan","whine, complain, cry"],
	["wife beater","tank top, sleeveless undershirt"],
	["wifebeater","tank top, sleeveless undershirt"],
	["ancient man","ancient civilization, ancient people"],
	["authoress","author, writer"],
	["average housewife","average consumer, average household, average homemaker"],
	["average man","average person"],
	["average working man","average wage earner, average taxpayer"],
	["aviatrix","aviator"],
	["brotherhood of man","the human family"],
	["calendar girl","model"],
	["call girl","escort, prostitute, sex worker"],
	["churchman","cleric, practicing Christian, pillar of the Church"],
	["english master","english coordinator, senior teacher of english"],
	["englishmen","the english"],
	["executrix","executor"],
	["father of *","founder of"],
	["founding father","the founders, founding leaders, forebears"],
	["housemaid","house worker, domestic help"],
	["industrial man","industrial civilization, industrial people"],
	["lady doctor","doctor"],
	["leading lady","lead"],
	["maiden","virgin"],
	["maiden race","first race"],
	["maiden speech","first speech"],
	["man a desk","staff a desk"],
	["man in the street","ordinary citizen, typical person, average person"],
	["man of the land","farmer, rural worker, grazier, landowner, rural community, country people, country folk"],
	["mans best friend","a faithful dog"],
	["man the booth","staff the booth"],
	["man the phones","answer the phones"],
	["mansized task","a demanding task, a big job"],
	["man sized task","a demanding task, a big job"],
	["master key","pass key, original"],
	["master copy","pass key, original"],
	["master plan","grand scheme, guiding principles"],
	["master the art","become skilled"],
	["men of science","scientists"],
	["midwife","birthing nurse"],
	["modern man","modern civilization, modern people"],
	["no mans land","unoccupied territory, wasteland, deathtrap"],
	["office girls","administrative staff"],
	["poetess","poet"],
	["railwayman","railway worker"],
	["sportsmanlike","fair, sporting"],
	["sportsmanship","fairness, good humor, sense of fair play"],
	["statesmanlike","diplomatic"],
	["statesman like","diplomatic"],
	["stockman","cattle worker, farmhand, drover"],
	["tradesmans entrance","service entrance"],
	["tax man","tax commissioner, tax office, tax collector"],
	["usherette","usher"],
	["woman lawyer","lawyer"],
	["woman painter","painter"],
	["working mother","wage or salary earning woman, two-income family"],
	["working wife","wage or salary earning woman, two-income family"],
	["homosexual","gay, gay man, lesbian, gay person/people"],
	["homosexual relations","relationship"],
	["homosexual relationship","relationship"],
	["homosexual couple","couple"],
	["sexual preference","sexual orientation, orientation"],
	["gay lifestyle","gay lives, gay/lesbian lives"],
	["homosexual lifestyle","gay lives, gay/lesbian lives"],
	["gay agenda","gay issues"],
	["homosexual agenda","gay issues"],
	["special rights","equal rights, civil rights for gay people"],
	["gay rights","equal rights, civil rights for gay people"],
	["fag","gay"],
	["faggot","gay"],
	["dyke","gay"],
	["homo","gay"],
	["sodomite","gay"],
	["bi","bisexual"],
	["homosexual marriage","gay marriage, same-sex marriage"],
	["tranny","transgender"],
	["transvestite","cross-dresser"],
	["sexchange","transition, gender confirmation surgery"],
	["sex change","transition, gender confirmation surgery"],
	["sex change operation","sex reassignment surgery, gender confirmation surgery"],
	["transgenders","transgender people"],
	["transgendered","transgender"],
	["transgenderism","being transgender, the movement for transgender equality"],
	["biologically male","assigned male at birth, designated male at birth"],
	["born a man","assigned male at birth, designated male at birth"],
	["genetically male","assigned male at birth, designated male at birth"],
	["biologically female","assigned female at birth, designated female at birth"],
	["born a woman","assigned female at birth, designated female at birth"],
	["genetically female","assigned female at birth, designated female at birth"],
	["bathroom bill","non-discrimination law, non-discrimination ordinance"],
	["hermaphroditic","intersex"],
	["pseudohermaphroditic","intersex"],
	["pseudo hermaphroditic","intersex"],
	["hermaphrodite","person who is intersex, person, intersex person"],
	["pseudohermaphrodite","person who is intersex, person, intersex person"],
	["pseudo hermaphrodite","person who is intersex, person, intersex person"],
	["shemale","transgender person, person"],
	["she male","transgender person, person"],
	["heshe","transgender person, person"],
	["shehe","transgender person, person"],
	["islamist","muslim, person of Islamic faith, fanatic, zealot, follower of islam, follower of the islamic faith"],
	["islamists","muslims, people of Islamic faith, fanatics, zealots"],
	["master","primary, primaries, hub, hubs, reference, references, replica, replicas, spoke, spokes, secondary, secondaries"],
	["masters","primary, primaries, hub, hubs, reference, references, replica, replicas, spoke, spokes, secondary, secondaries"],
	["slave","primary, primaries, hub, hubs, reference, references, replica, replicas, spoke, spokes, secondary, secondaries"],
	["slaves","primary, primaries, hub, hubs, reference, references, replica, replicas, spoke, spokes, secondary, secondaries"],
	["eskimo","Inuit"],
	["eskimos","Inuits"],
	["oriental","Asian person"],
	["orientals","Asian people"],
	["nonwhite","person of color, people of color"],
	["non white","person of color, people of color"],
	["ghetto","projects, urban"],
	["red indian","Native American"],
	["pocahontas","Native American"],
	["redskin","Native American"],
	["red indians","Native American People"],
	["redskins","Native American People"],
	["animal spirit","favorite, inspiration, personal interest, personality type"],
	["dream catcher","favorite, inspiration, personal interest, personality type"],
	["spirit animal","favorite, inspiration, personal interest, personality type"],
	["totem","favorite, inspiration, personal interest, personality type"],
	["long time no hear","I haven’t seen you in a long time, it’s been a long time"],
	["long time no see","I haven’t seen you in a long time, it’s been a long time"],
	["Indian country","enemy territory"],
	["jump the reservation","disobey, endure, object to, oppose, resist"],
	["off reserve","disobey, endure, object to, oppose, resist"],
	["off the reservation","disobey, endure, object to, oppose, resist"],
	["circle the wagons","defend"],
	["on the warpath","defend"],
	["too many chiefs","too many chefs in the kitchen, too many cooks spoil the broth"],
	["natives are restless","dissatisfied, frustrated"],
	["natives are becoming restless","dissatisfied, frustrated"],
	["natives are getting restless","dissatisfied, frustrated"],
	["natives are growing restless","dissatisfied, frustrated"],
	["pow wow","conference, gathering, meeting"],
	["powwow","conference, gathering, meeting"],
	["indian give","go back on one’s offer"],
	["indian giver","go back on one’s offer"],
	["pinoys","Filipinos, Filipino people"],
	["pinays","Filipinos, Filipino people"],
	["sand niggers","Arabs, Middle Eastern People"],
	["towel heads","Arabs, Middle Eastern People"],
	["latino","Latinx"],
	["latina","Latinx"],
	["mexican","Latinx"],
	["japs","Japanese person, Japanese people"],
	["shlomo","Jewish person"],
	["shyster","Jewish person"],
	["hymie","Jewish person"],
	["goyim","a person who is not Jewish, not Jewish"],
	["goyum","a person who is not Jewish, not Jewish"],
	["goy","a person who is not Jewish, not Jewish"],
	["spade","a Black person"],
	["gyppo","Nomad, Traveler, Roma, Romani"],
	["gypsy","Nomad, Traveler, Roma, Romani"],
	["Gipsy","Nomad, Traveler, Roma, Romani"],
	["gyp","Nomad, Traveler, Roma, Romani"],
	["blacklist","blocklist, wronglist, banlist, deny list"],
	["whitelist","passlist, alrightlist, safelist, allow list"],
	["primitive","simple, indigenous, hunter-gatherer"],
	["savage","simple, indigenous, hunter-gatherer"],
	["stone age","simple, indigenous, hunter-gatherer"],
	["tribe","society, community"],
	["sophisticated culture","complex culture"],
	["sophisticated technology","complex technology"],
	["make * great again","improve"],
	["make * * great again","improve"],
	["make * * * great again","improve"],
	["make * * * * great again","improve"],
	["make * * * * * great again","improve"],
	["committed suicide","died by suicide"],
	["completed suicide","died by suicide"],
	["commit suicide","die by suicide"],
	["complete suicide","die by suicide"],
	["successful suicide","die by suicide"],
	["suicide epidemic","rise in suicides"],
	["epidemic of suicides","rise in suicides"],
	["suicide pact","rise in suicides"],
	["failed suicide","suicide attempt, attempted suicide"],
	["failed attempt","suicide attempt, attempted suicide"],
	["suicide failure","suicide attempt, attempted suicide"],
	["suicide note","a note from the deceased"],
	["hang","the app froze, the app stopped responding, the app stopped responding to events, the app became unresponsive"],
	["hanged","the app froze, the app stopped responding, the app stopped responding to events, the app became unresponsive"]
]);